...
import com.hjy.sacore.base.support.WebClientSupport;
import com.hjy.sacore.baseclass.BaseClass;
import com.hjy.sacore.rsync.IRsyncConfig;
import com.hjy.sacore.rsync.IRsyncDateCheck;
import com.hjy.sacore.rsync.IRsyncDo;
...
import com.hjy.sacore.util.HexUtil;
import com.hjy.sacore.util.MD5Util;
...
public abstract class RsyncKjt<TConfig extends IRsyncConfig, TRequest extends IRsyncRequest, TResponse extends IRsyncResponse>
	...
	private MDataMap getsignMap(String requestStr) {
		...
		dataMap.put("sign", HexUtil.toHexString(MD5Util
				.md5(str.substring(0, str.toString().length() - 1) + "&" + bConfig("groupcenter.rsync_kjt_password"))));
		return dataMap;
	}
	...
	public String upLastSuccessStatusData() {
		...
		String sReturn = "";
		// MDataMap mLog =
		// DbUp.upTable("lc_rsync_kjt_log").oneWhere("status_data", "-zid", "",
		// "rsync_target",
	...
	public boolean doRsync() {
		...
		try {
			...
			// 插入日志表调用的日志记录
			mInsertMap.inAllValues("code", sCode, "rsync_target", upConfig().getRsyncTarget(), "rsync_url", sUrl,
			...
			// 更新处理完成时间
			mInsertMap.inAllValues("process_time", FormatHelper.upDateTime(), "process_data", rsyncResult.upJson(),
					"status_data", rsyncResult.getStatusData(), "flag_success", rsyncResult.upFlagTrue() ? "1" : "0",
					"process_num", String.valueOf(rsyncResult.getProcessNum()), "success_num",
			...
			mErrorMap.inAllValues("code", sCode, "flag_success", "0", "process_time", FormatHelper.upDateTime(),
