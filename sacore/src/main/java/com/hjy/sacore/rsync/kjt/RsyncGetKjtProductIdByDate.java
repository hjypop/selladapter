...
import com.hjy.sacore.base.support.MailSupport;
import com.hjy.sacore.rsync.config.kjt.RsyncConfigGetKjtProductIdByDate;
...
import com.hjy.sacore.rsync.request.kjt.RsyncRequestGetKjtProductIdByDate;
import com.hjy.sacore.rsync.response.kjt.RsyncResponseGetKjtProductIdByDate;
...
//					List<Map<String, Object>> downProductList = DbUp.upTable("pc_productinfo").dataSqlList(qSql,
//							new MDataMap());
					List<Map<String, Object>> downProductList = null;
					if (downProductList != null && downProductList.size() > 0) {
						...
						if (pCode1 != null && pCode1.size() > 0) {
							String sSql = "UPDATE pc_productinfo SET product_status=:product_status,flag_sale=:flag_sale where product_code_old in('"
									+ StringUtils.join(pCode1, "','")
									+ "')  and small_seller_code='SF03KJT' and seller_code='SI2003'";
//							DbUp.upTable("pc_productinfo").dataExec(sSql, upMap);// 商品下架
	 ...
	 *                          惠家有商品code @param pName 商品名称 @author 张海生 @date
	 *                          2015-8-21 下午2:09:49 @return void @throws
	 */
